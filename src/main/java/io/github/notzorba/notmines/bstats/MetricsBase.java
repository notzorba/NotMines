 ...
 */
package io.github.notzorba.notmines.bstats;
...
public final class MetricsBase {
    ...
    private void checkRelocation() {
        final String relocateCheck = System.getProperty("bstats.relocatecheck");
        if (relocateCheck != null && relocateCheck.equals("false")) {
            return;
        }

        final String defaultPackage = new String(new byte[] { 'o', 'r', 'g', '.', 'b', 's', 't', 'a', 't', 's' });
        final String examplePackage = new String(new byte[] { 'y', 'o', 'u', 'r', '.', 'p', 'a', 'c', 'k', 'a', 'g', 'e' });
        final String currentPackage = MetricsBase.class.getPackage().getName();
        if (currentPackage.startsWith(defaultPackage) || currentPackage.startsWith(examplePackage)) {
            throw new IllegalStateException("bStats Metrics class has not been relocated correctly!");
        }
    }

    private static byte[] compress(final String value) throws IOException {
        if (value == null) {
            return null;
        }
